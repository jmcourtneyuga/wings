...
class TestSqueezedGaussian:
    ...
    def test_positive_r_narrows(self):
        ...
        dx = g[1] - g[0]
        var_normal = np.sum(g**2 * np.abs(psi_normal)**2 * dx) / np.sum(np.abs(psi_normal)**2 * dx)
        var_squeezed = np.sum(g**2 * np.abs(psi_squeezed)**2 * dx) / np.sum(np.abs(psi_squeezed)**2 * dx)
