...
class TestRunResult:
    def test_run_result_creation(self):
        ...
        r = RunResult(
            run_id=0, fidelity=0.99, infidelity=0.01,
            params=np.zeros(10), circuit_std=0.1, circuit_mean=0.5,
            time_seconds=1.0, n_evaluations=100,
            strategy="smart", seed=42, success=True,
        )
    ...
    def test_run_result_failed(self):
        from wings.results import RunResult
        r = RunResult(
            run_id=1, fidelity=0.0, infidelity=1.0,
            params=None, circuit_std=0.0, circuit_mean=0.0,
            time_seconds=0.1, n_evaluations=0,
            strategy="random", seed=43,
...
class TestOptimizationResult:
    ...
    def test_dict_access(self):
        from wings.results import OptimizationResult
        r = OptimizationResult(
            optimal_params=np.zeros(10), fidelity=0.999, infidelity=0.001,
            time=5.0, n_evaluations=1000, success=True,
        )
        assert r["fidelity"] == 0.999
