...
def transfer_params(
    ...
    if n_source > n_target:
        raise ValueError(
            f"Source qubit count ({n_source}) cannot exceed target qubit count ({n_target})."
        )
    ...
    for layer in range(shared_layers):
        ...
        target_params[tgt_offset : tgt_offset + n_source] = source_params[
            src_offset : src_offset + n_source
        ]
    ...
    for layer in range(shared_layers, target_depth):
        tgt_offset = layer * n_target
        target_params[tgt_offset : tgt_offset + n_target] = np.random.randn(n_target) * init_scale
